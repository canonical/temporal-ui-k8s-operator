# Security Policy — Charmed Temporal UI

## Reporting a Vulnerability

The easiest way to report a security issue is through a [GitHub Private Security Report](https://github.com/canonical/temporal-ui-k8s-operator/security/advisories/new) with a description of the issue, the steps you took to create the issue, affected versions, and, if known, mitigations for the issue.
...
Charmed Temporal UI follows Canonical's standard charm support lifecycle, aligned with the Ubuntu LTS base.
...
Vulnerabilities are tracked via [GitHub Security Advisories](https://github.com/canonical/temporal-ui-k8s-operator/security/advisories) and coordinated internally with Canonical's Product Security Incident Response Team (PSIRT).

Note that this charm packages the upstream [Temporal Web UI](https://github.com/temporalio/ui-server). Vulnerabilities in the upstream UI server itself should be reported to the Temporal project directly; Canonical will track and apply upstream fixes to supported tracks.
